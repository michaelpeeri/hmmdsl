...
def main_test(model_file, fasta_file, args = None):
    ...
    #relax_mtx = hmmdsl_py.Matrix();
    #hmmdsl_py.read_matrix("jones_relax.mat", relax_mtx);

    
    #if isinstance(model, hmmdsl_py.Model):
    #    hmmdsl_py.relax_emissions(model, 0.03, relax_mtx, "ARNDCQEGHILKMFPSTWYV")
    #else:
    #    hmmdsl_py.relax_emissions_hmm(model, 0.03, relax_mtx, "ARNDCQEGHILKMFPSTWYV")

    #model.debug_print()
    if isinstance(model, hmmdsl_py.Model):
        hmmdsl_py.align_viterbi(fasta_file, model, [], 1 )
    else:
        #hmmdsl_py.align_viterbi_hmm(fasta_file, model, [] )
        assert(False)
